 ...
 * @param {string} prompt - The prompt text to be sent to the Gemini model.
 * @param {string} filePath - The filepath of the file to be used for the prompt
 * @param {string}
 * @returns {Promise<string>} - The response content generated by the model.
 */
export async function sendPromptInline(schema, prompt, filePath, fileExtension) {
    let attempt = 0;
    const MAX_RETRIES = 3; // Maximum retry attempts
    const BASE_DELAY = 1000; // Initial delay in ms (1 second)
    ...
    while (attempt < MAX_RETRIES) {
        try {
            logger.info(`Attempt ${attempt + 1} to send inline prompt...`);

            const fileType = getMimeType(fileExtension);

            if (!fileType) {
                logger.info(`Unsupported file type. Please provide a valid file extension.`);
                throw new Error("Unsupported file type. Please provide a valid file extension.");
            }
